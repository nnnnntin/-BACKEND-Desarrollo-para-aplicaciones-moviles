...
const getProveedoresByServicioController = async (req, res) => {
  ...
  try {
    ...
    if (servicios.length === 0) {
      return res.status(404).json({ 
        ...
        details: `No se encontraron servicios del tipo: ${tipoServicio}`
      });
    }
    ...
    if (error.name === 'ReferenceError' && error.message.includes('Proveedor')) {
      return res.status(500).json({ 
        message: "Error en la configuración", 
        details: "Modelo de Proveedor no importado correctamente"
      });
    }
    ...
    res.status(500).json({ 
      message: "Error al obtener proveedores por servicio", 
      details: error.message
    });
  }
};
...
const verificarProveedorController = async (req, res) => {
  ...
  try {
    // Primero verificamos que existe el proveedor
    ...
    // Verificamos si el proveedor ya está verificado
    ...
    // Guardamos los documentos de verificación (en una implementación real esto iría a otra colección)
    // y luego verificamos el proveedor
...
const filtrarProveedoresController = async (req, res) => {
  ...
  try {
    ...
    // Manejar el filtro de servicioOfrecido de manera especial
    if (value.servicioOfrecido) {
      try {
        // Importa el modelo ServicioAdicional al inicio del archivo
        const ServicioAdicional = require("../models/servicioAdicional.model");
        
        // Primero encuentra los servicios que coinciden con el tipo
        const servicios = await ServicioAdicional.find({ tipo: value.servicioOfrecido });
        
        if (servicios.length === 0) {
          return res.status(404).json({ 
            message: "Tipo de servicio no encontrado", 
            details: `No se encontraron servicios del tipo: ${value.servicioOfrecido}`
          });
        }
        ...
        if (error.name === 'Error' && error.message.includes('Servicio')) {
          return res.status(500).json({ 
            message: "Error en la configuración", 
            details: "Modelo de ServicioAdicional no importado correctamente"
          });
        }
        throw error;
      }
    ...
    // Si no hay filtro por servicio, usa la función estándar
