...
const getFacturasByEmisorController = async (req, res) => {
  const { emisorId, tipoEmisor } = req.query;
  
  if (!emisorId) {
    return res.status(400).json({ 
      message: "Parámetro requerido", 
      details: "Se requiere el ID del emisor"
    });
  }
  
  // Validar tipo de emisor si está presente
  if (tipoEmisor && !['empresa', 'propietario', 'proveedor'].includes(tipoEmisor)) {
    return res.status(400).json({ 
      message: "Tipo de emisor inválido", 
      details: "El tipo de emisor debe ser 'empresa', 'propietario' o 'proveedor'"
    });
  }
  
  try {
    const facturas = await getFacturasByEmisor(emisorId, tipoEmisor);
    res.status(200).json(facturas);
  } catch (error) {
    console.error(error);
    ...
    if (error.name === 'CastError') {
      return res.status(400).json({ 
        message: "ID de emisor inválido", 
        details: `El formato del ID '${emisorId}' no es válido`
      });
    }
    ...
    if (error.message && (error.message.includes('no encontrado') || error.message.includes('not found'))) {
      return res.status(404).json({ 
        ...
        details: `No existe un emisor con id: ${emisorId} y tipo: ${tipoEmisor || 'cualquiera'}`
      });
    }
    
    res.status(500).json({ 
      message: "Error al obtener facturas por emisor", 
      details: error.message 
    });
  }
};
...
const cambiarEstadoFacturaController = async (req, res) => {
  const { id } = req.params;
  ...
  try {
    const factura = await cambiarEstadoFactura(id, estado);
    if (!factura) {
      return res.status(404).json({ message: `No se ha encontrado la factura con id: ${id}` });
    }
    res.status(200).json({ message: "Estado actualizado correctamente", factura });
  } catch (error) {
    console.error(error);
...
const marcarFacturaComoPagadaController = async (req, res) => {
  const { id } = req.params;
  
  try {
    const factura = await marcarFacturaComoPagada(id);
    if (!factura) {
      return res.status(404).json({ message: `No se ha encontrado la factura con id: ${id}` });
    }
    res.status(200).json({ message: "Factura marcada como pagada", factura });
  } catch (error) {
    console.error(error);
    ...
    if (error.message && error.message.includes('ya pagada') || error.message.includes('already paid')) {
      return res.status(400).json({ 
        message: "Factura ya pagada", 
        details: "La factura ya se encuentra en estado pagada"
      });
    }
    ...
    if (error.message && error.message.includes('cancelada') || error.message.includes('cancelled')) {
      return res.status(400).json({ 
        ...
        details: "No se puede marcar como pagada una factura cancelada"
      });
    }
    
    if (error.message && error.message.includes('pago insuficiente') || error.message.includes('insufficient payment')) {
      return res.status(400).json({ 
        message: "Pago insuficiente", 
        details: "El monto de los pagos asociados no cubre el total de la factura"
      });
    }
...
const marcarFacturaComoVencidaController = async (req, res) => {
  const { id } = req.params;
  
  try {
    const factura = await marcarFacturaComoVencida(id);
    if (!factura) {
      return res.status(404).json({ message: `No se ha encontrado la factura con id: ${id}` });
    }
    res.status(200).json({ message: "Factura marcada como vencida", factura });
  } catch (error) {
    console.error(error);
    ...
    if (error.message && error.message.includes('ya vencida') || error.message.includes('already expired')) {
      return res.status(400).json({ 
        message: "Factura ya vencida", 
        details: "La factura ya se encuentra en estado vencida"
      });
    }
    ...
    if (error.message && error.message.includes('pagada') || error.message.includes('paid')) {
      return res.status(400).json({ 
        ...
        details: "No se puede marcar como vencida una factura pagada"
      });
    }
    ...
    if (error.message && error.message.includes('cancelada') || error.message.includes('cancelled')) {
      return res.status(400).json({ 
        message: "Factura cancelada", 
        details: "No se puede marcar como vencida una factura cancelada"
      });
    }
    
    if (error.message && error.message.includes('fecha vencimiento') || error.message.includes('due date')) {
      return res.status(400).json({ 
        message: "Fecha de vencimiento futura", 
        details: "No se puede marcar como vencida una factura cuya fecha de vencimiento aún no ha llegado"
      });
    }
    
    res.status(500).json({ 
      message: "Error al marcar la factura como vencida", 
      details: error.message 
    });
  }
};
...
const getEstadisticasFacturacionController = async (req, res) => {
  const { fechaInicio, fechaFin } = req.query;
  ...
  try {
    ...
    const estadisticas = await getEstaditicasFacturacion(inicio, fin);
...
const generarPdfFacturaController = async (req, res) => {
  const { error, value } = generarPdfFacturaSchema.validate(req.body);
  if (error) {
    return res.status(400).json({ 
      message: "Error de validación", 
      details: error.details[0].message,
  ...
  const { facturaId, plantillaId, incluyeLogo, idioma } = value;
  
  try {
    // Primero verificamos que existe la factura
    const factura = await findFacturaById(facturaId);
    if (!factura) {
      return res.status(404).json({ message: `No se ha encontrado la factura con id: ${facturaId}` });
    }
    
    // En un caso real, aquí se generaría el PDF
    // Este es solo un ejemplo, en la implementación real se conectaría con un servicio de generación de PDFs
    
    // Simulamos una URL de PDF generada
    const pdfUrl = `https://ejemplo.com/facturas/pdf/${facturaId}?t=${new Date().getTime()}`;
    
    // Actualizamos la URL en la factura
    const facturaActualizada = await actualizarPdfUrl(facturaId, pdfUrl);
    
    res.status(200).json({ 
      message: "PDF generado correctamente", 
      factura: facturaActualizada,
      pdfUrl,
      plantilla: plantillaId || 'default',
      incluyeLogo: incluyeLogo || false,
      idioma: idioma || 'es'
    });
  } catch (error) {
    console.error(error);
    ...
    if (error.name === 'CastError') {
      return res.status(400).json({ 
        message: "ID de factura inválido", 
        details: `El formato del ID '${facturaId}' no es válido`
      });
    }
    
    if (error.message && error.message.includes('plantilla no encontrada') || error.message.includes('template not found')) {
      return res.status(404).json({ 
        message: "Plantilla no encontrada", 
        details: `No se encontró la plantilla con id: ${plantillaId}`
      });
    }
