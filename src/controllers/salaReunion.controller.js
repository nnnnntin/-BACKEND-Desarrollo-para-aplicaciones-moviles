...
const getSalasReunionController = async (req, res) => {
  try {
    const salas = await getSalasReunion();
...
const agregarEquipamientoController = async (req, res) => {
  ...
  if (!equipamiento || !equipamiento.tipo) {
    return res.status(400).json({ 
      message: "Datos inválidos", 
      details: "Se requiere información de equipamiento con un tipo"
    });
  }
  
  try {
    const sala = await agregarEquipamiento(id, equipamiento);
    if (!sala) {
      return res.status(404).json({ message: `No se ha encontrado la sala de reunión con id: ${id}` });
    }
    res.status(200).json({ message: "Equipamiento agregado correctamente", sala });
  } catch (error) {
    console.error(error);
...
const actualizarPreciosController = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ message: "Precios actualizados correctamente", sala });
  } catch (error) {
    console.error(error);
