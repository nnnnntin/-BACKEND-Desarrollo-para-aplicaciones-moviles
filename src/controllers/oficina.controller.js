...
const getOficinasDisponiblesController = async (req, res) => {
  ...
  if (horaInicio) {
    const formatoHoraRegex = /^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/;
    if (!formatoHoraRegex.test(horaInicio)) {
      return res.status(400).json({ 
  ...
  if (horaFin) {
    const formatoHoraRegex = /^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/;
    if (!formatoHoraRegex.test(horaFin)) {
      return res.status(400).json({ 
        message: "Formato de hora inválido", 
  ...
  // Validar que horaInicio sea anterior a horaFin si se proporcionan ambas
...
const filtrarOficinasController = async (req, res) => {
  ...
  try {
    ...
    if (value.precioMaximo) {
      ...
      // Asumimos que el precio es por día por defecto
